...
Я решил выполнить первые 3 варианта задания из методички и при этом скомбинировать 2-й и 3-й вариант.
...
1. Программа может запускаться как с аргументами, так и без них:
	* Без аргументов программа будет выполнять поиск и копирование изображений из текущей директории и поддиректориях.
	* С аргументом(путь до папки) программа будет выполнять те же действия, но из заданной пользователем директории.
2. Программа будет иметь проверки на верность аргументов.
...
6. По завершению работы будет выводиться отчет о выполнении(кол-во скопированных файлов и путь до папки резервного копирования).
...
Я заранее подготовил изображения ```.jpg``` формата и сохранил их на виртуальной машине в ```~/Изображения```.

Так как изображения и [скрипт](/lab3/backup_images.sh) находятся в разных директориях я указал аргумент при запуске:  
...
Как и было задумано программа повторяет структуру исходных файлов:
* Исходная папка:  
...
1. Программа может запускаться как с аргументами, так и без них:
	* Если без то будут использоваться параметры по умолчанию  
	```SOURCE_DIR``` — исходная папка (по умолчанию: текущая)  
	```OUTPUT_DIR``` — выходная папка (авто имя по дате)  
...
3. Будет происходить рекурсивный поиск ```.jpg``` и ```.jpeg``` файлов в ```исходной папке``` и её поддиректориях:  
	...
	2. Извлекается дата из EXIF(случаи с отсутствием даты были учтены).
...
Как и говорилось, в коде будет использоваться программа ```convert``` пакета ```ImageMagick```.
...
помимо простого получения и добавление на изображение даты из EXIF, а так же конвертации в формат ```JPEG2000```, я:
...
> [!NOTE]  
> Судя по содержимому сообщений ```WARNING``` начиная с 7-й версии ImageMagick вместо программы ```convert``` следует использовать ```magick```.  
...
<div align="center">
<table>
<tr>Количество файлов изображений было довольно большим, поэтому в репозиторий я загрузил далеко не все:</tr>
<tr><td>До</td><td>После</td></tr><tr><td>
<img src="/lab3/images-jpg/ES-room.jpg"/></td><td>
<img src="/lab3/images-jp2/ES-room.jpg"/></td></tr>
<tr><td>До</td><td>После</td></tr><tr><td>
<img src="/lab3/images-jpg/bg/mike_room_night.jpg"/></td><td>
<img src="/lab3/images-jp2/bg/mike_room_night.jpg"/></td></tr>
<tr><td>До</td><td>После</td></tr><tr><td>
<img src="/lab3/images-jpg/bg/room_mc_night_light.jpg"/></td><td>
<img src="/lab3/images-jp2/bg/room_mc_night_light.jpg"/></td></tr>
<tr><td>До</td><td>После</td></tr><tr><td>
<img src="/lab3/images-jpg/bg/school_roof_day_puddle.jpg"/></td><td>
<img src="/lab3/images-jp2/bg/school_roof_day_puddle.jpg"/></td></tr>
<tr><td>До</td><td>После</td></tr><tr><td>
<img src="/lab3/images-jpg/bg/tokyo_street_night.jpg"/></td><td>
<img src="/lab3/images-jp2/bg/tokyo_street_night.jpg"/></td></tr>
<tr><td>До</td><td>После</td></tr><tr><td>
<img src="/lab3/images-jpg/bg/garden_sunset.jpg"/></td><td>
<img src="/lab3/images-jp2/bg/garden_sunset.jpg"/></td></tr>
</table>
</div>

> [!IMPORTANT]  
> Так как предпросмотр изображений формата ```JPEG2000``` на GitHub не поддерживается, 
